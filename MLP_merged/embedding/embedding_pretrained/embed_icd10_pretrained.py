#!/usr/bin/env python3
"""Create one pretrained ICD-10 embedding per four-field PCR sample."""
...
def sample_codes(columns):
  """Combine the four symptom/impression fields into one sample-level sequence."""
  return [
    code
    for column_idx in CODE_COLUMN_INDEXES
    for code in code_list(columns[column_idx])
  ]
...
def build_embeddings(args):
  ...
  for row_idx, (_header, columns) in enumerate(iter_rows(args.input_file)):
    ...
    embeddings[row_idx] = mean_code_embedding(
      sample_codes(columns), code_to_index, code_embedding_table, hidden_size
    )
  ...
  save_metadata(
    ...
    args.prefix,
    {
      "encoding": "pretrained_icd10_sample_embedding",
      "input_file": args.input_file,
      "model_name": args.model_name,
      "shape": list(output_shape),
      "dtype": "float32",
      "pooling": "attention_mask_mean_pooling",
      "sample_pooling": "mean_of_all_icd10_code_embeddings_across_four_fields",
  ...
  save_metadata(args.output_dir, args.prefix + "_code_vocab", {"vocab": code_to_index})
  print("wrote merged pretrained ICD-10 embeddings:", output_path)
...
def parse_args():
  parser = argparse.ArgumentParser(
    description="Create one pretrained ICD-10 embedding from four NEMSIS symptom/impression fields."
  )
