...
def main():
  ...
  parser.add_argument("--output_dir", default="/home/xiangling/EMSAssist/MLP_separated/dataset/word_bag")
  ...
  x = sparse.load_npz(source / "word_bag_embeddings.npz").tocsr().astype(np.float32)
  labels = np.asarray(read_lines(source / "word_bag_protocol_labels.txt"))
  keys = np.asarray(read_lines(source / "word_bag_pcr_keys.txt"))
  ...
  metadata = {
    "source_embedding": str(source / "word_bag_embeddings.npz"),
    "seed": args.seed,
