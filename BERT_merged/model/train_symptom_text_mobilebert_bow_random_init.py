"""Train a randomly initialized MobileBERT with bag-of-words input encoding.

The four symptom/impression fields of each sample are treated as one sentence.
Token counts are projected through MobileBERT's randomly initialized word-
embedding matrix and mean-pooled before being passed to MobileBERT as a
one-token sequence.
"""
...
DEFAULT_ARGUMENTS = {
    ...
    "--model_path": (
        ...
        "symptom_text_mobilebert_bow_random_init.pt"
    ),
    "--evaluation_path": (
        "/home/xiangling/EMSAssist/BERT_merged/evaluation/"
        "symptom_text_mobilebert_bow_random_init_metrics.txt"
    ),
}
