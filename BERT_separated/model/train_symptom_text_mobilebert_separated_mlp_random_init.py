"""Train randomly initialized MobileBERT on four separately encoded symptom fields."""
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, index):
        row = self.rows[index]
        field_texts = tuple(row[1:5])
        return field_texts, int(self.labels[index])
...
class TextCollator:
    ...
    def __call__(self, samples):
        field_groups, labels = zip(*samples)
        flat_texts = [text for fields in field_groups for text in fields]
        encoded = self.tokenizer(flat_texts, padding=True, truncation=True,
                                 max_length=self.max_length, return_tensors="pt")
        batch_size = len(samples)
        encoded = {
            name: tensor.reshape(batch_size, NUM_FIELDS, tensor.shape[-1])
...
class SeparatedMobileBertWithMLP(nn.Module):
    """Shared MobileBERT encoder for four fields followed by an MLP classifier."""
    ...
    def __init__(self, model_name, num_classes, local_files_only=True):
        ...
        self.mobilebert = AutoModel.from_config(config)
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(NUM_FIELDS * hidden_size, hidden_size),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None, **kwargs):
        ...
        if num_fields != NUM_FIELDS:
            raise ValueError(f"Expected {NUM_FIELDS} fields, got {num_fields}")
        flat_input_ids = input_ids.reshape(batch_size * num_fields, sequence_length)
        flat_attention_mask = (
            attention_mask.reshape(batch_size * num_fields, sequence_length)
            if attention_mask is not None else None
        )
        flat_token_type_ids = (
            token_type_ids.reshape(batch_size * num_fields, sequence_length)
        ...
        field_embeddings = outputs.pooler_output.reshape(batch_size, num_fields, -1)
        concatenated_embedding = field_embeddings.reshape(batch_size, -1)
...
def new_model(args, num_classes):
    return SeparatedMobileBertWithMLP(
...
def main():
    ...
    parser.add_argument("--model_path", type=Path, default=Path("/home/xiangling/EMSAssist/BERT_separated/model/symptom_text_mobilebert_separated_mlp_random_init.pt"))
    parser.add_argument("--evaluation_path", type=Path, default=Path("/home/xiangling/EMSAssist/BERT_separated/evaluation/symptom_text_mobilebert_separated_mlp_random_init_metrics.txt"))
    ...
    torch.save({"model_state_dict": model.state_dict(), "model_name": args.model_name,
                "architecture": "SeparatedMobileBertWithMLP", "num_classes": metadata["num_classes"],
    ...
    lines = ["Randomly Initialized Separated Symptom Text MobileBERT + MLP Evaluation", "=====================================================================", "",
