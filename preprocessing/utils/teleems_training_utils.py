...
def get_model_path(saved_model_dir, 
    ...
    if do_quantity:
        task_name += "_quantity"
    if do_procedure:
        task_name += "_procedure"

    best_model_path = os.path.join(saved_model_dir, best_model_name + task_name + ".pt")
    print("[prearrival_training_utils] according to your config, best_model_path:", best_model_path)
...
def print_evaluate_results(args, results):
    ...
    for model_name in results.keys():
        ...
        if args.do_procedure:
            procedure_accuracy = results[model_name]["procedure"]["accuracy"]
            procedure_top1_accuracy = results[model_name]["procedure"]["hamming_loss"]
            procedure_top3_accuracy = results[model_name]["procedure"]["f1_micro"]
            procedure_top5_accuracy = results[model_name]["procedure"]["f1_macro"]
            print(f"[training_utils] Procedure - Accuracy: {procedure_accuracy:.4f}, hamming_loss: {procedure_top1_accuracy:.4f}, f1_micro: {procedure_top3_accuracy:.4f}, f1_macro: {procedure_top5_accuracy:.4f}")
