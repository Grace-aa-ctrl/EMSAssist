...
import utils.teleems_training_utils as tu
...
def train_emsglass(train_dataloader,
    ...
    tasks = {
        ...
        "procedure": {"do_task": args.do_procedure, "metrics": {
            "f1": evaluate.load("f1"),
    ...
    for model_name, (model, required_inputs) in model_input_map.items():
        ...
        learning_rate = 5e-5
        print(f"[train_emsglass] begins training {model_name}, learning rate {learning_rate} ...")
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_dataloader:
                ...
                protocol_logits, med_type_logits, quantity_output, procedure_logits, *_ = model(**inputs)
                ...
                if tasks["procedure"]["do_task"]:
                    labels = batch[procedure_label_name]  # shape: [batch, num_labels]

                    # print(f"[debug] procedure_logits shape in train_emsglass: ", procedure_logits.shape)
                    # print(f"[debug] labels shape in train_emsglass: ", labels.shape)
                    # print(f"[debug] procedure_logits in train_emsglass: ", procedure_logits)
                    # print(f"[debug] labels in train_emsglass: ", labels)

                    procedure_loss = tasks["procedure"]["loss_fn"](procedure_logits, labels)
                    batch_loss += procedure_loss

                batch_loss.backward()

                # print("Before:", model.layer.weight.data[0][0])
                optimizer.step()
                # print("After:", model.layer.weight.data[0][0])
            ...
            elif args.do_procedure:
                val_accuracy = evaluate_results[model_name]["procedure"]["f1_macro"]  # Use macro-F1 score
            else:
            ...
            if val_accuracy > best_val_accuracy:
                ...
                print(f"Best {model_name} model saved in {best_model_path} with validation accuracy: {val_accuracy if (args.do_protocol or args.do_med_type or args.do_procedure) else -val_accuracy}")
...
def evaluate_emsglass(test_dataloader, 
    ...
    for model_name, (model, required_inputs) in model_input_map.items():
        ...
        tasks = {
            ...
            # "procedure": {"do_task": args.do_procedure, "metrics": {"f1": evaluate.load("f1"), "precision": evaluate.load("precision"), "recall": evaluate.load("recall")}}
            "procedure": {"do_task": args.do_procedure, "y_true": [], "y_pred": []}
        }
        ...
        for batch in test_dataloader:
            ...
            if tasks["procedure"]["do_task"]:
                preds = (procedure_logits > 0.5).int().cpu().numpy()  # threshold at 0 (assuming BCEWithLogitsLoss)
                labels = batch[procedure_label_name].cpu().numpy()
                tasks["procedure"]["y_pred"].extend(preds)
                tasks["procedure"]["y_true"].extend(labels)
                # # Apply sigmoid to get probabilities
                # procedure_probs = torch.sigmoid(procedure_logits)
                # # Convert to binary predictions (threshold = 0.5)
                # procedure_preds = (procedure_probs > 0.5).int()
                # label = batch[procedure_label_name].to(torch.int32)
                
                # # The metrics expect lists for multi-label classification
                # predictions_list = procedure_preds.cpu().numpy().tolist()
                # references_list = label.cpu().numpy().tolist()
                
                # # Calculate metrics
                # for name, metric in tasks["procedure"]["metrics"].items():
                #     if name == "f1":
                #         # F1 score - default is macro average
                #         print(f"[debug] predictions shape {procedure_preds.shape}, reference {label.shape}")
                #         metric.add_batch(predictions=predictions_list, references=references_list)
                #     else:  # precision and recall
                #         metric.add_batch(predictions=predictions_list, references=references_list)
        ...
        for task_name, task_info in tasks.items():
            if task_info["do_task"]:
                if task_name == "quantity":
                    ...
                    y_true = np.array(task_info["y_true"])
                    y_pred = np.array(task_info["y_pred"])
                    ...
                    # procedure_results = {}
                    # for name, metric in task_info["metrics"].items():
                    #     if name == "f1":
                    #         procedure_results["f1"] = metric.compute(average="macro")["f1"]
                    #     elif name == "precision":
                    #         procedure_results["precision"] = metric.compute(average="macro")["precision"]
                    #     elif name == "recall":
                    #         procedure_results["recall"] = metric.compute(average="macro")["recall"]
                    # model_results[task_name] = procedure_results
                else:
...
def predict_single_text(model, text_encoder_name, text, device, max_seq_len=128):
    model.eval()

    # Tokenize input
    tokenized_text_ids = tokenize_function(text_encoder_name, text, max_seq_len)
    tokenized_text_id_tensor = {k: torch.tensor(v).to(device) for k, v in tokenized_text_ids.items()}
    # print("tokenized_text_id_tensor: ", tokenized_text_id_tensor)

    input_ids = tokenized_text_id_tensor['input_ids'].unsqueeze(0)                            # [1, 64]
    attention_mask = tokenized_text_id_tensor['attention_mask'].unsqueeze(0)                  # [1, 64]
    token_type_ids = tokenized_text_id_tensor['token_type_ids'].unsqueeze(0)                  # [1, 64]
    # print("input_ids: ", input_ids)

    # Forward pass (adjust for model’s input structure)
    with torch.no_grad():
        single_text_predictions = model(input_ids=input_ids, attention_mask=attention_mask,token_type_ids=token_type_ids)

    # print("single_text_predictions: ", single_text_predictions)
    # Print all prediction dimensions
    if isinstance(single_text_predictions, tuple) or isinstance(single_text_predictions, list):
        print("Model output is a tuple/list with {} elements".format(len(single_text_predictions)))
        for i, out in enumerate(single_text_predictions):
            if isinstance(out, torch.Tensor):
                print(f"Output[{i}] shape: {out.shape}")
            else:
                print(f"Output[{i}] is of type {type(out)}")
    elif isinstance(single_text_predictions, torch.Tensor):
        print(f"Model output shape: {single_text_predictions.shape}")
    else:
        print(f"Model output is of type: {type(single_text_predictions)}")

    return single_text_predictions

def predict_teleems_to_be_deleted(args,
                    protocol_label_name,
                    med_type_label_name,
                    quantity_label_name,
                    procedure_label_name,
...
def predict_teleems(args, multimodal_ems_model, text_inputs=None, vital_inputs=None):
    # model = multimodal_ems_model.to(f"cuda:{args.device}").eval()
    model = multimodal_ems_model.to(args.device).eval()

    input_dict = {}
    if text_inputs:
        input_dict.update({
            # "input_ids": torch.tensor(text_inputs["input_ids"]).unsqueeze(0).to(f"cuda:{args.device}"),
            # "attention_mask": torch.tensor(text_inputs["attention_mask"]).unsqueeze(0).to(f"cuda:{args.device}"),
            # "token_type_ids": torch.tensor(text_inputs["token_type_ids"]).unsqueeze(0).to(f"cuda:{args.device}")
            "input_ids": torch.tensor(text_inputs["input_ids"]).unsqueeze(0).to(args.device),
            "attention_mask": torch.tensor(text_inputs["attention_mask"]).unsqueeze(0).to(args.device),
            "token_type_ids": torch.tensor(text_inputs["token_type_ids"]).unsqueeze(0).to(args.device)
        })
    if vital_inputs is not None:
        # input_dict["vital"] = torch.tensor(vital_inputs).unsqueeze(0).float().to(f"cuda:{args.device}")
        input_dict["vital"] = torch.tensor(vital_inputs).unsqueeze(0).float().to(args.device)
    ...
    #         if isinstance(out, torch.Tensor):
    #             print(f"[predict_teleems] Output[{i}] shape: {out.shape}")
    # else:
    #     print(f"[predict_teleems] Output shape: {outputs.shape if isinstance(outputs, torch.Tensor) else type(outputs)}")
    ...
    return {
        ...
        "quantity": outputs[2],
        "procedure": outputs[3],
        "text_output": outputs[4] if len(outputs) > 4 else None,
        "vital_output": outputs[5] if len(outputs) > 5 else None,
        "scene_output": outputs[6] if len(outputs) > 6 else None,
        "combined_output": outputs[7] if len(outputs) > 7 else None,
    }


def teleems_predict_postprocessor(pred_results, my_np, med_supertype_list, med_norm_params, my_pp, procedure_pred_threshold, do_protocol=True, do_med_type=True, do_quantity=True, do_procedure=True):

    teleems_pred_dict = {}
    if do_protocol:
        protocol_index = pred_results['protocol'].item()
        protocol_id = my_np.dedicated_protocols[protocol_index]
        protocol_str = my_np.nemsis_protocol_id2desc[protocol_id]
        print("protocol_id = %s" % protocol_index)
        print("protocol = %s" % protocol_str)
        teleems_pred_dict['Protocol'] = protocol_str

    if do_med_type:
        med_type_index = pred_results['med_type'].item()
        med_type_desc = med_supertype_list[med_type_index]
        print("med_type_index = %s" % med_type_index)
        print("med_type = %s" % med_type_desc)
        teleems_pred_dict['Medicine'] = med_type_desc

    if do_quantity:
        normed_quantity = pred_results['quantity'].item()
        mean = med_norm_params['type2quantities_mean'][str(med_type_index)]
        std = med_norm_params['type2quantities_std'][str(med_type_index)]
        quantity = normed_quantity * std + mean
        print("normed_quantity = %s" % normed_quantity)
        print("quantity = %s" % quantity)
        teleems_pred_dict['Quantity'] = str(round(quantity, 1))

    # text_transcription is a string list of length 1, so we need to extract the string
    if do_procedure:
        procedure_pred_threshold = procedure_pred_threshold  # Convert to a scalar
        procedure_probs = pred_results['procedure'].squeeze(0)    # pred_results['procedure'] is a 2D tensor with shape [1, num_procedures (18)]
        # print("procedure_probs = %s" % procedure_probs)
        thresholded_procedure_probs = (procedure_probs > procedure_pred_threshold).int()
        # print("thresholded_procedure_probs = %s" % thresholded_procedure_probs)
        # Get predicted class indices
        predicted_indices = torch.nonzero(thresholded_procedure_probs, as_tuple=True)[0].tolist()  # e.g., [0, 2, 5]
        # print("predicted_indices = %s" % predicted_indices)
        # print("my_pp.supertype_id2desc = %s" % my_pp.supertype_id2desc)
        predicted_procedure_names = ", ".join([my_pp.supertype_id2desc[idx] for idx in predicted_indices])
        # print(f"Predicted procedure names: {predicted_procedure_names}")

        teleems_pred_dict['Procedure'] = predicted_procedure_names

    return teleems_pred_dict
