...
cached_pcr2si_protocol_med_vital_file_name = "nemsis_pcr2si_protocol_med_ppgvital.txt"                             
cached_meta_vitals_mean_file_name = "meta_ppgvitals_mean.txt"
...
class Vital_Processor(object):
  ...
  def __init__(self, args):
    ...
    self.cached_meta_vitals_mean_file_path = os.path.join(self.cache_dir, self.nemsis_year, cached_meta_vitals_mean_file_name)
  ...
  def get_pcr2vitals(self):
    ...
    util.writeDictToFile(self.cached_meta_vitals_mean_file_path, valid_vitals_mean_dict)
  ...
  def sort_and_normalize_vitals(self,
    ...
    for vital_index in self.vitals_of_interest:
      ...
      for pcr_k, vitals_dict in per_pcr_vitals_dict.items():  
        ...
        pcr2vitals_dict[pcr_k][vital_index] = [chronologically_sorted_vitals] + [mean_filled_vitals] + [forward_backward_filled_vitals]
  ...
  # def get_si_protocol_med_vitals(self, pcr2vitals, args):
    ...
    # the vital files has 13 columns + number of vitals * 3 columns = 13 + 6 * 3 = 31 columns
    if os.path.isfile(self.cached_pcr2si_protocol_med_vital_file_path):
      return util.read_dict_from_file(self.cached_pcr2si_protocol_med_vital_file_path, line_length=31, is_value_a_set=False, discard_header=True)
    pcr2vitals = self.get_pcr2vitals()
    print("pcr2vitals size: ", len(pcr2vitals))
    ...
    for i in range(len(self.vitals_of_interest)):
      ...
      for f in vital_filling:
        # for n in vital_norms:
            vitals_headers.append("%s_vital_%s" % (f, i))           # refer to the buttom of get_cached_pcr2si_protocol_med_vitals
    ...
    with open(self.mp.cached_pcr2si_protocol_med_file_path, "r") as r_f, open(self.cached_pcr2si_protocol_med_vital_file_path, "w") as w_f:
      for row, line in enumerate(r_f):
        ...
        if pcr_k in pcr2vitals:
          ...
          assert len(vital_examples) == len(self.vitals_of_interest) * 3
  ...
  def get_vital_examples(self, vitals_dict):
    ...
    vital_examples = []
    for vital_index in self.vitals_of_interest:
      unnormed_vitals = vitals_dict[vital_index]
      assert len(unnormed_vitals) == 3           # for ppg vitals, we have 1 original vital and 2 filled vitals, 6*3 = 18 columns
      # vital_example = []
      for idx, vitals in enumerate(unnormed_vitals):
        assert len(vitals) == len(unnormed_vitals[0])
