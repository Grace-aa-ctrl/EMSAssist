...
cached_pcr2si_header = "PcrKey~|~PrimarySymptomCode~|~PrimaryImpressionCode~|~AdditionalSymptomCode~|~SecondaryImpressionCode"
...
cached_pcr2protocol_header = "PcrKey~|~ProtocolId"
cached_pcr2si_protocol_file_name = "nemsis_pcr2si_protocol.txt"
cached_pcr2si_protocol_header = cached_pcr2si_header + "~|~ProtocolIds"
...
class NEMSIS_Processor(object):
  ...
  def __init__(self, nemsis_dir, data_dir, cache_dir, nemsis_year):
    ...
    pri_sym_ref = self._nemsis_file_path(pri_sym_ref_name)
    pri_imp_ref = self._nemsis_file_path(pri_imp_ref_name)
    add_sym_ref = self._nemsis_file_path(add_sym_ref_name)
    sec_imp_ref = self._nemsis_file_path(sec_imp_ref_name)
    ...
    self.cached_pcr2si_file_path = self._cache_file_path(cached_pcr2si_file_name)
    self.cached_pcr2protocol_file_path = self._cache_file_path(cached_pcr2protocol_file_name)
    self.cached_pcr2si_protocol_file_path = self._cache_file_path(cached_pcr2si_protocol_file_name)
  ...
  def _nemsis_file_path(self, file_name):

    candidates = []
    if self.nemsis_year:
      candidates.append(os.path.join(self.nemsis_dir, self.nemsis_year, file_name))
    candidates.append(os.path.join(self.nemsis_dir, file_name))
  ...
  def _cache_file_path(self, file_name):

    cache_subdir = os.path.join(self.cache_dir, self.nemsis_year) if self.nemsis_year else self.cache_dir
    os.makedirs(cache_subdir, exist_ok=True)
    return os.path.join(cache_subdir, file_name)

  def _file_has_header(self, file_path):

    if not os.path.isfile(file_path):
      return False

    with open(file_path, "r", encoding="utf-8", errors="replace") as r_f:
      first_line = r_f.readline().strip().lower()
    return first_line.startswith("pcrkey")
...
#    writeListFile("sorted_words_list.txt", words_list)
  ...
  def get_pcr2si(self):
    ...
    if os.path.isfile(self.cached_pcr2si_file_path):
      return util.read_dict_from_file(self.cached_pcr2si_file_path, line_length=5, discard_header=self._file_has_header(self.cached_pcr2si_file_path))

    ps_file_path = self._nemsis_file_path(fact_pcr_ps_file_name)
    pi_file_path = self._nemsis_file_path(fact_pcr_pi_file_name)
    as_file_path = self._nemsis_file_path(fact_pcr_as_file_name)
    si_file_path = self._nemsis_file_path(fact_pcr_si_file_name)
    ...
    with open(self.cached_pcr2si_file_path, "w", encoding="utf-8") as w_f:
      w_f.write(cached_pcr2si_header + "\n")
      for pcr, si in pcr2si.items():
        line = pcr + si + "\n"
        w_f.write(line)
    print("write %s lines to %s" % (len(pcr2si), self.cached_pcr2si_file_path))
    return util.read_dict_from_file(self.cached_pcr2si_file_path, line_length=5, discard_header=True)
  
  def augment_pcr_with_si(self, pcr_set, input_si_file_path, pcr2si):

    pcr_accessed = set()
    with open(input_si_file_path, "r", encoding="utf-8", errors="replace") as r_f:
  ...
  def get_pcr2protocol_set(self):
    ...
    if os.path.isfile(self.cached_pcr2protocol_file_path):
      return util.read_dict_from_file(self.cached_pcr2protocol_file_path, is_value_a_set=True, discard_header=self._file_has_header(self.cached_pcr2protocol_file_path))

    nemsis_protocol_file_path = self._nemsis_file_path(fact_pcr_protocol_file_name)
    protocol_file_write_count = 0
    with open(nemsis_protocol_file_path, "r", encoding="utf-8", errors="replace") as r_f, open(self.cached_pcr2protocol_file_path, "w", encoding="utf-8") as w_f:
      w_f.write(cached_pcr2protocol_header + "\n")
      for idx, line in enumerate(r_f):
        if idx == 0:
          print("skipping line at 0:", line)
          continue
    ...
    return util.read_dict_from_file(self.cached_pcr2protocol_file_path, is_value_a_set=True, discard_header=True)

  def get_pcr2si_protocols(self):

    if os.path.isfile(self.cached_pcr2si_protocol_file_path):
      return util.read_dict_from_file(self.cached_pcr2si_protocol_file_path, line_length=6, discard_header=self._file_has_header(self.cached_pcr2si_protocol_file_path))
    ...
    protocol_file_write_count = 0
    pcr2si_has_header = self._file_has_header(self.cached_pcr2si_file_path)
    with open(self.cached_pcr2si_file_path, "r", encoding="utf-8", errors="replace") as r_f, open(self.cached_pcr2si_protocol_file_path, "w", encoding="utf-8") as w_f:
      w_f.write(cached_pcr2si_protocol_header + "\n")
      for idx, line in enumerate(r_f):
        if idx == 0 and pcr2si_has_header:
          continue
    ...
    return util.read_dict_from_file(self.cached_pcr2si_protocol_file_path, line_length=6, discard_header=True)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--nemsis_dir", action='store', type=str, default = "/data_8TB_2/xiangling_workspace/NEMSIS_ASCII_25")
  parser.add_argument("--nemsis_year", action='store', type=str, default = "")
  parser.add_argument("--data_folder", action='store', type=str, default = "/home/xiangling/EMSAssist/preprocessing/data")
  parser.add_argument("--cache_dir", action='store', type=str, default = "/home/xiangling/nemsis_cache_files")
