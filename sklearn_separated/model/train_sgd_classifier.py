#!/usr/bin/env python3
"""Tune and evaluate SGDClassifier on the pre-split ICD-10 bag data."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/home/xiangling/EMSAssist/sklearn_separated/evaluation/sgd_classifier_metrics.txt"),
    )
    parser.add_argument(
        "--alpha-values", nargs="+", type=float, default=[1e-5, 1e-4, 1e-3]
    )
...
def make_model(alpha: float, args: argparse.Namespace) -> SGDClassifier:
...
def top_k_accuracy(model: SGDClassifier, x, y, k: int) -> float:
...
def main() -> None:
    args = parse_args()
    if any(alpha <= 0 for alpha in args.alpha_values):
    ...
    for alpha in args.alpha_values:
        started = time.time()
        model = make_model(alpha, args)
        model.fit(train_x, train_y)
        score = float(accuracy_score(val_y, model.predict(val_x)))
        elapsed = time.time() - started
        tuning.append(
            {"alpha": alpha, "validation_accuracy": score, "seconds": elapsed}
        )
        print(
            f"alpha={alpha:g}: validation accuracy={score:.8f} ({elapsed:.1f}s)",
            flush=True,
        )
        if score > best_score:
    ...
    print(
        f"Refitting with alpha={best_alpha:g} on train + validation...", flush=True
    )
    final_model = make_model(best_alpha, args)
    ...
    lines.extend(
        f"alpha={row['alpha']:g}: accuracy={row['validation_accuracy']:.8f}, "
        f"seconds={row['seconds']:.1f}"
        for row in tuning
    )
    lines.extend(
        [
            f"best_alpha: {best_alpha:g}",
            ...
            "JSON",
            json.dumps(
                {"best_alpha": best_alpha, "validation": tuning, "test": metrics},
                indent=2,
            ),
        ]
    )
