#!/usr/bin/env python3
"""Tune and evaluate a linear SVM on the pre-split ICD-10 bag data."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/home/xiangling/EMSAssist/sklearn_separated/evaluation/svm_metrics.txt"),
    )
    parser.add_argument("--c-values", nargs="+", type=float, default=[0.01, 0.1, 1.0])
...
def make_model(c: float, args: argparse.Namespace) -> LinearSVC:
    return LinearSVC(
        C=c,
        penalty="l2",
        loss="squared_hinge",
...
def top_k_accuracy(model: LinearSVC, x, y, k: int) -> float:
...
def main() -> None:
    ...
    for c in args.c_values:
        ...
        tuning.append({"C": c, "validation_accuracy": score, "seconds": elapsed})
    ...
    lines.extend(
        f"C={row['C']:g}: accuracy={row['validation_accuracy']:.8f}, "
